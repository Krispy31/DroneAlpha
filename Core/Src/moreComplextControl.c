...
void PIDController (void) {
    ...
    // cover this directional stuff
    float angleZ = 0.0f;
    float angleXY = 0.0f;// this probs works differently
    float fullForce = 100.0f;
    float zForce = 100.0f;
    float xyForce = 0.0f;
    float xForce = 0.0f;
    float yForce = 0.0f;
    ...
    zForce = fullForce*sin(angleZ);
    xyForce = fullForce*cos(angleZ);
    xForce = xyForce*sin(angleZ);
    yForce = xyForce*cos(angleZ);

    // from here, I have the force in each direction, which should be proportional to acceleration in each direction
    // this is what is to be controlled,
    // the other thing is, its ok to have an angle when you are trying to move in that direction, but not in annother direction
